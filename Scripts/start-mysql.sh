...
recover_mysql_credentials() {
        echo "Configured MySQL credentials do not match the existing mysql_data volume."
        echo "Resetting only the local root and application passwords; database data will be preserved."

        docker stop local-mysql > /dev/null
        docker rm local-mysql > /dev/null

        docker run -d \
            --name mysql-credential-recovery \
        ...
        for i in {1..30}; do
                if docker exec mysql-credential-recovery mysql -uroot -e 'SELECT 1' > /dev/null 2>&1; then
                        break
                fi
                if [ "$i" -eq 30 ]; then
                        docker rm -f mysql-credential-recovery > /dev/null 2>&1 || true
                        exit 1
                fi
                sleep 1
        done

        docker exec mysql-credential-recovery mysql -uroot -e "
            FLUSH PRIVILEGES;
        ...
        docker rm -f mysql-credential-recovery > /dev/null
        docker run -d \
            --name local-mysql \
            ...
            for i in {1..30}; do
                ...
                if [ "$i" -eq 30 ]; then
                    echo "Timeout waiting for MySQL after credential recovery" >&2
                    exit 1
                fi
                sleep 1
            done
...
if docker ps --format '{{.Names}}' | grep -q '^local-mysql$'; then
    ...
    for i in {1..30}; do
        ...
        if [ $i -eq 30 ]; then
            echo "MySQL is running but configured credentials were rejected; attempting credential recovery."
            break
        fi
        echo "Waiting... ($i/30)"
        sleep 1
